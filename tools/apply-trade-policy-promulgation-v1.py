...
def replace(path, old, new):
    ...
    if old not in s:
        raise SystemExit(f'missing snippet in {path}: {old[:120]!r}')
    p.write_text(s.replace(old,new,1))

# Trade policy: foreign import restrictions/tariffs normally take effect only
# after a physical notice can reach the counterpart. Domestic export controls
# remain immediate because they bind our own merchants.
p='js/economy/tradePolicy.js'
replace(p,"import { changeAttitude } from '../diplomacy/relations.js?v=20260904-save1';\n",
...
replace(p,"export function setTradeRestriction(region, { direction = 'trade', goods = null, counterparties = null,\n  allowed = false, tariffRate = 0 } = {}, regions = [], currentTick = null) {",
...
replace(p,"  const oldTariffRate = Math.max(0, Number(oldRule?.tariffRate) || 0);\n",
...
"""
replace(p,old,new)
replace(p,"    tariffRate: cleanTariffRate,\n    changedTick: currentTick,\n    harmByActor,\n",
...
# Trade runtime: departures use the terms known at dispatch; arrivals grandfather
# pre-notice cargo unless the issuing state chose immediate border enforcement.
p='js/economy/trade.js'
replace(p,"import { borderTariffQuote, tradeAllowed } from './tradePolicy.js?v=20260905-policy1';",
          "import { borderTariffQuote, tradeAllowed, tickTradePolicyCommunications } from './tradePolicy.js?v=20260905-policy1';")
replace(p,"function findOpportunities(region, candidateRegions, knownIdsByRegion, pricesByRegion, regionsById, regions, agreements, transitContext = null) {",
...
replace(p,"      const tariffQuote = borderTariffQuote(region, dest, resource);",
          "      const tariffQuote = borderTariffQuote(region, dest, resource, 0, { currentTick, departureTick: currentTick });")
replace(p,"        const tariffQuote = borderTariffQuote(origin, dest, venture.resource);",
          "        const tariffQuote = borderTariffQuote(origin, dest, venture.resource, 0, { currentTick, departureTick: venture.departureTick });")
replace(p,"          const settledTariff = borderTariffQuote(origin, dest, venture.resource, goodsValue);",
...
replace(p,"export function tickTrade(regions, currentTick = null, time = null, agreements = [], profiler = null) {\n",
...
replace(p,"    const opportunities = findOpportunities(region, candidates, knownIdsByRegion, pricesByRegion, regionsById, regions, agreements, transitContext);",
...
# Treasurer UI: normal communicated enforcement is the default; immediate is explicit.
p='js/ui/advisors.js'
replace(p,
...
assert.equal(borderTariffQuote(exporter,importer,'steel',0,{currentTick:10,departureTick:10}).allowed,true);
assert.equal(borderTariffQuote(exporter,importer,'steel',0,{currentTick:notice,departureTick:notice}).allowed,false);
assert.equal(borderTariffQuote(exporter,importer,'steel',0,{currentTick:notice+5,departureTick:10}).allowed,true,'pre-notice cargo must be grandfathered');
...
assert.equal(borderTariffQuote(urgentExporter,urgentImporter,'steel',0,{currentTick:20,departureTick:15}).allowed,false,'immediate enforcement reaches cargo already underway');
...
assert.equal(borderTariffQuote(tariffExporter,tariffImporter,'steel',100,{currentTick:30,departureTick:30}).importTariff,0);
assert.equal(borderTariffQuote(tariffExporter,tariffImporter,'steel',100,{currentTick:tariffNotice,departureTick:tariffNotice}).importTariff,25);
