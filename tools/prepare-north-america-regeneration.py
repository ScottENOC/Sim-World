#!/usr/bin/env python3
"""Remove prior North America v1 generated output before regenerating it.

The North America builder is additive by design. Once its generated files are
committed, rerunning it would otherwise see its own regions as occupied land.
This preflight removes only output owned by this tranche, preserving the world
that existed before North America v1 and the pre-existing Bering Sea.
"""
import json
...
SEA_PLAN_PATH = ROOT / 'tools' / 'north-america-sea-plan-v1.json'
SOURCE_GROUP = 'na_north_america_v1'
# Bering Sea predates this tranche and must never be deleted by its preflight.
PREEXISTING_SEA_IDS = {'sea_bering'}
...
def main():
    ...
    if removed_land:
        geo['features'] = [
            ...
            if f.get('properties', {}).get('id') not in removed_land
        ]
    ...
    owned_seas = {spec['id'] for spec in sea_plan.get('regions', [])} - PREEXISTING_SEA_IDS
    ...
    sea_geo['features'] = [
        f for f in sea_geo.get('features', [])
        if f.get('properties', {}).get('id') not in owned_seas
    ]
    ...
    print(f'RESET_NORTH_AMERICA_LAND={len(removed_land)}')
    print(f'RESET_NORTH_AMERICA_SEAS={len(present_seas)}')
