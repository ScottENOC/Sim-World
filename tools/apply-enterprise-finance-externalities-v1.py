...
def replace_once(path,before,after):
    ...
    if before not in s: raise RuntimeError(f'{path}: anchor not found')
...
# Apply externalities and maintenance standard in tick.
...
import { enterpriseCostExternalityProfile, enterpriseDebtCapacity, ensureEnterpriseOperatingModel } from '../js/economy/enterpriseBehaviour.js';
...
let cap=enterpriseDebtCapacity(e,{sovereignCredit:.9,revenueReliability:.95});
...
assert.equal(build.accepted,true); assert(e.cash>=0); assert(e.investedCapital===190);
assert(build.asset.operatingModel);
...
assert(bad.apparentCostSaving>good.apparentCostSaving);
assert(bad.labourHarm>good.labourHarm); assert(bad.environmentalHarm>good.environmentalHarm); assert(bad.futureLiability>good.futureLiability);
...
assert(region.enterpriseExternalities.environmentalHarm>0);
assert(build.asset.lastMaintenanceRatio<1);
