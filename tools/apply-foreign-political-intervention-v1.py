...
# Model: add foreign-backed revolutionary/coup network funding.
replace_once(
    'js/politics/foreignPoliticalIntervention.js',
    "function uprisingCandidate(exilePolity, targetPolity, regions) {",
    r'''export function ensureDestabilisationOperation(targetPolity, sponsorPolityId, mode = 'revolution') {
...
replace_once(
    ...
    r'''function interventionTargets(home, regions, options = {}) {
...
replace_once(
    ...
    r'''  const rerender = () => { section.remove(); renderDiplomaticServicePanel(container, home, regions, currentTick, options); };
...
replace_once(
    ...
    r'''    const foreignInterventionEvents = profiler.measure('Foreign political intervention', () =>
...
replace_once(
    ...
    r'''  if (region.id === playerRegionId) renderDiplomaticServicePanel(document.getElementById('region-controls'), region, regions, calendarWeekIndex(clock.elapsedDays || 0), {
    polities,
...
replace_once(
    ...
    r'''  if (event.type === 'foreign_restoration_support_detected') {
...
Path('tools/test-foreign-political-intervention.mjs').write_text(r'''import assert from 'node:assert/strict';
...
# CI coverage.
replace_once('.github/workflows/institutional-actions.yml',
             "'feature/regime-change-*']",
             "'feature/regime-change-*', 'feature/foreign-political-intervention-*']")
replace_once('.github/workflows/institutional-actions.yml',
             "      - 'js/politics/regimeChange.js'",
             "      - 'js/politics/regimeChange.js'\n      - 'js/politics/foreignPoliticalIntervention.js'")
# diplomaticServicePanel already appears in PR paths; no duplicate path insertion.
replace_once('.github/workflows/institutional-actions.yml',
...
print('foreign political intervention integration applied')
