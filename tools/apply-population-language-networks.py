...
# The old courier regression used a scalar region languageId to assert an already-shared
# diplomatic language. Express the same scenario using the new specialist pool instead.
p = Path('tools/test-joint-operations-command.mjs')
text = p.read_text()
old = "import { ensureCommunicationState } from '../js/diplomacy/languageCommunication.js';"
new = "import { ensureCommunicationState } from '../js/diplomacy/languageCommunication.js';\nimport { adoptInstitutionalLanguage, ensureLanguageNetwork } from '../js/diplomacy/languageNetworks.js';"
if old in text:
    text = text.replace(old, new, 1)
old = """ensureCommunicationState(sender).languageId = 'lang:diplomatic-test';\nensureCommunicationState(ally).languageId = 'lang:diplomatic-test';"""
new = """ensureCommunicationState(sender); ensureCommunicationState(ally);\nfor (const court of [sender, ally]) {\n  ensureLanguageNetwork(court).specialists['lang:diplomatic-test'] = { conversational: 8, working: 5, fluent: 3, literate: 0, scribes: 0, interpreters: 2 };\n  adoptInstitutionalLanguage(court, 'court', 'lang:diplomatic-test');\n}"""
if old in text:
    text = text.replace(old, new, 1)
p.write_text(text)
