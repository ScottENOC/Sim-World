...
USER_AGENT = 'Sim-World sea expansion v2/1.0'
...
def main():
    ...
    occupied = repair(unary_union([repair(shape(f['geometry'])) for f in sea_geo.get('features', [])]))
    ...
    for spec in plan['regions']:
        ...
        if not occupied.is_empty:
            water = repair(water.difference(occupied))
        if water.is_empty or area_sqkm(water) < 100:
            print(f"SEA_SKIP {spec['id']} already covered or no substantial water")
            continue
        c = water.centroid
        new_features.append({'type':'Feature','properties':{'id':spec['id'],'name':spec['name']},'geometry':mapping(water)})
        sea_meta['seaRegions'].append({'id':spec['id'],'name':spec['name'],'centroid':[c.x,c.y],'areaSqKm':area_sqkm(water),'adjacentLand':[]})
        occupied = repair(unary_union([occupied, water]))
        print(f"SEA_ADD {spec['name']} area={area_sqkm(water):.0f}")
    sea_geo['features'].extend(new_features)
    ...
    land = [(f['properties']['id'], f['properties'].get('name',''), repair(shape(f['geometry']))) for f in land_geo.get('features', [])]
    sea_feature_by_id = {f['properties']['id']: f for f in sea_geo['features']}
    for meta in sea_meta['seaRegions']:
        feature = sea_feature_by_id.get(meta['id'])
        if feature is None:
            continue
    ...
    review = [{'id':m['id'],'name':m['name'],'adjacentLandCount':len(m.get('adjacentLand',[])),'areaSqKm':round(m.get('areaSqKm',0),1)} for m in sea_meta['seaRegions']]
