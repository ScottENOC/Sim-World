...
def main():
    ...
    still = []
    for rid in new_ids:
        if not meta_by_id[rid].get('neighbors') and rid not in coastal:
            still.append((rid, meta_by_id[rid].get('name', rid)))
    if still:
        for rid, name in still:
            geom = geoms[index_by_id[rid]]
            nearest = sorted((geom.distance(other), oid, meta_by_id.get(oid, {}).get('name', oid), oid in new_ids)
                             for oid, other in zip(ids, geoms) if oid != rid)
            ...
            for distance, oid, oname, isnew in nearest[:8]:
                print(f'ORPHAN_NEAREST distanceDegrees={distance:.6f} new={int(isnew)} id={oid} name={oname}')
