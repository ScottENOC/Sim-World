...
# Main integration: build once after land/sea adjacency exists, then regional rendering
# consumes the shared graph rather than inventing physical features independently.
...
# There are two region-opening paths: direct map selection and council navigation.
replace_once('js/main.js',
...
# second occurrence after council setup
...
# Saved-game restore rebuilds the graph's mutable site references from restored place state.
replace_once('js/main.js',
...
# When the already-open region sheet is reconstructed after load, restore its zoom affordance too.
replace_once('js/main.js',
...
# HTML full-screen local view. Canvas remains cheap: it only draws the selected region.
...
# Appended styles avoid disturbing existing strategic-map layout.
...
# Version marker for browser cache.
