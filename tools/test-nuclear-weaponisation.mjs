...
for(let i=260;i<320;i++) tickNuclearWeaponProgramme(r,i,7,()=>0);
assert.equal(state.tests.length,0,'a test policy and prototype are not enough without physical test infrastructure');
addTestRange(r);
for(let i=320;i<480&&!state.tests.length;i++) tickNuclearWeaponProgramme(r,i,7,()=>0);
assert.ok(state.tests.some(t=>t.publiclyDeclared),'public-test policy with a test range should eventually produce an unmistakable demonstration');
assert.equal(nuclearDeterrentStatus(r),'demonstrated_device_capability');
const observer=region('observer');
...
const covert=region('covert');covert.unlockedTechIds.add(NUCLEAR_WEAPONISATION_TECH_ID);covert.unlockedTechIds.add(NUCLEAR_TEST_VALIDATION_TECH_ID);addTestRange(covert);
...
assert.ok(cs.tests.length>0,'covert testing should be possible with physical test infrastructure');
