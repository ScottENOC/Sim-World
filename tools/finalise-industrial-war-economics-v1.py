...
# Fix modern weapon syntax and manufactured ammunition handling typo.
p = Path('js/military/modernLandWarfare.js')
t = p.read_text()
t = t.replace('(mg?.12:0)', '(mg ? .12 : 0)').replace('(mg?.34:0)', '(mg ? .34 : 0)')
p.write_text(t)

# Make reconstruction reflect currently damaged infrastructure, including rail,
# and make munitions production require fiscal spending as well as materials.
...
"""
if old not in t:
    raise RuntimeError('produceMunitions anchor missing')
t = t.replace(old, new)
...
# Tick war economy before trade so current fighting immediately disrupts routes,
# while state finance later in the tick can respond by borrowing for the next cycle.
p = Path('js/main.js')
t = p.read_text()
...
if "profiler.measure('Industrial war economy'" not in t:
    if anchor not in t: raise RuntimeError('main war economy tick anchor missing')
    t = t.replace(anchor, replacement, 1)
p.write_text(t)

# Strengthen regression around fiscal production and repairable reconstruction.
