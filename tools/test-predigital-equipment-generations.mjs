...
import { EQUIPMENT_FAMILIES, ensureCurrentArmouredVehicleDesign, ensureCurrentArtilleryDesign, designsFor } from '../js/military/equipmentGenerations.js';
...
function industrialRegion(id='r'){
  return {id,name:id,unlockedTechIds:new Set(['steelmaking','breech_loading_artillery','quick_firing_artillery','heavy_howitzers','advanced_factories']),
    ...
    industrialProduction:{factorySophistication:.7},industrialSupply:{capability:{precision_machining:.2},inventory:{machine_components:10000}},stockpile:{steel:20000,wood:20000,iron:5000,coal:5000,gunpowder:1000},
...
for(const q of [.12,.34,.56,.78,.98]){
  ...
  ensureCurrentArmouredVehicleDesign(land,EQUIPMENT_FAMILIES.SELF_PROPELLED_GUN);
  ensureCurrentArtilleryDesign(land,'field_cannon');
  ensureCurrentArtilleryDesign(land,'heavy_howitzer');
}
assert(designsFor(land,EQUIPMENT_FAMILIES.TANK).length>=4,'shared component progress should naturally create several tank Marks');
assert(designsFor(land,EQUIPMENT_FAMILIES.SELF_PROPELLED_GUN).length>=4,'SPGs should progress through several Marks');
assert(designsFor(land,EQUIPMENT_FAMILIES.FIELD_ARTILLERY).length>=3,'towed artillery should progress through multiple physical models');
assert(designsFor(land,EQUIPMENT_FAMILIES.HEAVY_ARTILLERY).length>=3,'heavy artillery should progress through multiple physical models');

// Actual vehicle production is stamped into a design-specific inventory; later Marks do not upgrade earlier output.
...
addProductionLine(prod,{productId:'tank',capacityShare:1});prod.industrialOrders.tank=5;tickIndustrialPlants(prod,7);
...
setComponents(prod,.9);prod.industrialPlants.productExperience.tank=.8;for(const c of Object.keys(ps.componentInventory))ps.componentInventory[c]=1000;prod.industrialOrders.tank=5;tickIndustrialPlants(prod,7);
const ids=Object.keys(prod.militaryEquipment.inventoryByDesign);assert(ids.length>=2,'improved components should cause later production to use a newer tank Mark');
...
// Every persistent ship class can acquire several pre-digital Marks from better hulls, machinery, guns, optics and analogue electrical systems.
