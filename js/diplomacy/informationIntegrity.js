...
export function ensureInformationIntegrity(region){
  ...
  for(const [key,value] of Object.entries({
    ...
    foreignInfluencePressure:0,lastIncidentTick:-Infinity,
...
export function assessPublicClaim(observer,claim={}){
  ...
  return {
    ...
    verdict:confidence>.76?'well_corroborated':confidence>.52?'probable':confidence>.30?'uncertain':'poorly_supported',
...
export function recordInformationIncident(region,incident={}){
  const s=ensureInformationIntegrity(region);
  const entry={
    id:incident.id||`info-${Date.now()}-${s.incidents.length}`,
    ...
    competingNarratives:Math.max(1,Number(incident.competingNarratives)||1),
  };
