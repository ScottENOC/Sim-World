...
function comms(r){return clamp((r.unlockedTechIds?.has?.('electrical_telegraphy')?.25:0)+(r.unlockedTechIds?.has?.('telephone_networks')?.16:0)+(r.unlockedTechIds?.has?.('radio_broadcasting')?.18:0)+(r.unlockedTechIds?.has?.('printing_press')?.12:0)+(r.communicationState?.messengerExperience||0)/800);}
...
export function worldInstitutionReadiness(polities,regions,activeWars=[]){
  ...
  const sharedProblem=clamp(rs.reduce((n,r)=>n+(r.unlockedTechIds?.has?.('anthropogenic_climate_change')?1:0),0)/Math.max(1,rs.length)*.65 + rs.reduce((n,r)=>n+(r.externalities?.knowledge?.ozone_depletion?.recognised?1:0),0)/Math.max(1,rs.length)*.35);
...
export function proposeInternationalOrganisation({proposerPolityId,name='International Organisation',level='secretariat',charter={},hostPolityId=null},world,currentTick=0){
  ...
  for(const p of polities){const isMajor=majors.includes(p.id);const rs=territories(p,regions);const connectivity=weighted(rs,comms);const acceptance=clamp(metrics.legitimacy*.46+readiness.demand*.20+connectivity*.12+admin(p,rs)*.10+(isMajor?metrics.greatPowerBuyIn*.22:.08));if(acceptance> (isMajor?.44:.36))members.push(p.id);}
  const populationTotal=regions.reduce((n,r)=>n+(r.population||0),0);const memberPop=regions.filter(r=>members.includes(polityId(r))).reduce((n,r)=>n+(r.population||0),0);const acceptance=clamp((members.length/Math.max(1,polities.length))*.55+(memberPop/Math.max(1,populationTotal))*.45);
...
function memberSupport(org,polity,motion,world){const rs=territories(polity,world.regions);const legitimacy=org.metrics?.legitimacy||0;const capacity=admin(polity,rs);let interest=.45;
  if(motion.type==='climate_action')interest=clamp(rs.some(r=>r.unlockedTechIds?.has?.('anthropogenic_climate_change'))?.72:.24);
  if(motion.type==='pollution_control')interest=clamp(rs.some(r=>Object.values(r.externalities?.knowledge||{}).some(k=>k?.recognised))?.68:.28);
...
export function submitInternationalMotion(org,{proposerPolityId,type,strength=.5,targetPolityId=null},world,currentTick=0){if(!org?.active||!org.memberPolityIds.includes(proposerPolityId)||!MOTION_TYPES[type])return{submitted:false,reason:'invalid_motion'};const spec=MOTION_TYPES[type];const votes=[];let yes=0,total=0;for(const id of org.memberPolityIds){const p=world.polities.find(x=>x.id===id);if(!p)continue;let weight=1+(org.charter?.weightedVoting||0)*(org.majorPowerIds.includes(id)?1.5:0);const support=memberSupport(org,p,{type,strength,targetPolityId},world);const veto=org.charter?.vetoMemberIds?.includes?.(id)&&support<.42;votes.push({polityId:id,support,weight,veto});total+=weight;if(support>=spec.difficulty)yes+=weight;if(veto){const motion={id:`motion-${nextMotionId++}`,type,status:'vetoed',proposerPolityId,targetPolityId,strength,votes,currentTick};org.motions.push(motion);return{submitted:true,passed:false,motion};}}
