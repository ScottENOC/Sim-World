// Graded inland/coastal navigation. Natural river geometry/capacity remains the
// baseline, while hydrology can now change effective flow through seasonality,
// withdrawals, storage and releases. Trade/military systems consume the result
// without creating a second transport simulation.
...
function hydrologyState(river,regionId=null,explicit=null){
...
export function riverNavigationState(river,{currentDay=0,latitude=0,weatherIndex=0,vesselDraftM=0.5,vesselDisplacementT=5,channelEngineering=0,iceManagement=0,regionId=null,waterState=null}={}){
  const nav=river?.navigation||{};
  const engineering=clamp(channelEngineering);
  const h=hydrologyState(river,regionId,waterState);
  const flowRatio=clamp(h?.flowRatio??h?.minFlowRatio??1,0.08,1.75);
  // Low flow reduces usable depth strongly. High controlled releases help, but
  // cannot make the natural channel arbitrarily deep without physical works.
  const flowDepthFactor=clamp(Math.sqrt(flowRatio),0.28,1.22);
  const draftCapacity=Math.max(0.2,(Number(nav.baseMaxDraftM)||0.8)*(1+engineering*0.55)*flowDepthFactor);
  const displacementCapacity=Math.max(1,(Number(nav.baseMaxDisplacementT)||20)*(1+engineering*1.25)*clamp(flowRatio,0.2,1.3));
  ...
  const capacity=clamp((Number(nav.naturalCapacity)||0.35)*(0.72+engineering*0.55)*(1-adverseFlow)*clamp(flowRatio,0.15,1.25));
  return {
    vesselFit,
    flowRatio,
    ...
    speedMultiplier:clamp(vesselFit*ice.navigationMultiplier*(1-adverseFlow)*clamp(flowRatio,0.25,1.1),0.02,1),
