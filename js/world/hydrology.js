...
function pollutionSources(region,elapsedDays){const monthScale=Math.max(.01,positive(elapsedDays)/30),population=positive(region?.population),farmers=positive(region?.report?.farming?.workers),manure=positive(region?.livestockAgriculture?.manureNutrientLoad),drainage=hasAsset(region,'urban_drainage'),legacyDrainageControl=drainage?.12:0,wasteFactor=wastewaterPollutionMultiplier(region),farmControls=clamp(region?.waterPolicy?.agriculturalRunoffControl??0),industrialControls=clamp(region?.waterPolicy?.industrialDischargeControl??0),sewageControl=clamp(Math.max(legacyDrainageControl,1-wasteFactor)),pathogen=population*.0000022*(1-sewageControl)*monthScale,cropNutrients=farmers*.0000014,livestockNutrients=manure*.006,nutrients=(cropNutrients+livestockNutrients)*(1-farmControls*.82)*monthScale,chemicals=positive(region?.industrialChemicalDischarge)*(1-industrialControls*.92)*monthScale;return{pathogen,nutrients,chemicals};}
...
function eutrophicationRisk(c){return clamp(c.nutrients*2.2+c.chemicals*.25,0,1);}
function ecologicalStress(flowRatio,risk,eutrophication){const lowFlow=clamp(Math.max(0,.75-flowRatio)/.75);return clamp(lowFlow*.62+clamp(risk)*.30+clamp(eutrophication)*.48);}
...
export function tickHydrology(graph,regions=[],currentDay=0,elapsedDays=30){
  ...
  for(const region of regions){const h=ensureRegionalHydrology(region);prepareRegionalWaterDemand(region,elapsedDays);h.report={surfaceInflow:0,surfaceOutflow:0,surfaceWithdrawal:0,managedRelease:0,waterHealthRisk:0,eutrophicationRisk:0,ecologicalStress:0,riverCount:0,floodPeakReduction:0,hydropowerPotential:0};}
  for(const river of graph.corridors.values()){
    ...
    for(let i=0;i<segments.length;i++){
      ...
      pollution=decayPollution(pollution,elapsedDays/Math.max(1,segments.length));pollution=addPollution(pollution,pollutionSources(region,elapsedDays));const c=concentration(pollution,outflow),risk=healthRisk(c),eutrophication=eutrophicationRisk(c),flowRatio=outflow/Math.max(.05,naturalInflow),ecoStress=ecologicalStress(flowRatio,risk,eutrophication),h=ensureRegionalHydrology(region);h.report.surfaceInflow+=naturalInflow;h.report.surfaceOutflow+=outflow;h.report.surfaceWithdrawal+=withdrawal;h.report.managedRelease+=Math.max(0,-regulated.storageChange);h.report.waterHealthRisk=Math.max(h.report.waterHealthRisk,risk);h.report.eutrophicationRisk=Math.max(h.report.eutrophicationRisk,eutrophication);h.report.ecologicalStress=Math.max(h.report.ecologicalStress,ecoStress);h.report.floodPeakReduction=Math.max(h.report.floodPeakReduction,regulated.floodPeakReduction||0);h.report.hydropowerPotential+=regulated.hydropowerPotential||0;h.report.riverCount+=1;h.waterHealthRisk=h.report.waterHealthRisk;h.waterborneDiseasePressure=clamp(risk*.45*(region?.urbanWater?.waterborneRiskMultiplier??1));
      river.hydrology.segments[region.id]={regionId:region.id,naturalInflow,outflow,flowRatio,withdrawal,stored:regulated.stored,storageChange:regulated.storageChange,floodPeakReduction:regulated.floodPeakReduction||0,hydropowerPotential:regulated.hydropowerPotential||0,pollutionLoad:{...pollution},concentration:c,waterHealthRisk:risk,eutrophicationRisk:eutrophication,ecologicalStress:ecoStress};flowSum+=outflow;minFlowRatio=Math.min(minFlowRatio,flowRatio);
      if(previousRegion){const quantityHarm=clamp((1-flowRatio)*.7),pollutionHarm=clamp(Math.max(risk*.8,eutrophication*.7)),quantityEvent=applyDiplomaticExternality(region,previousRegion,quantityHarm,'upstream_water_reduction',currentDay),pollutionEvent=applyDiplomaticExternality(region,previousRegion,pollutionHarm,'upstream_water_pollution',currentDay);if(quantityEvent)events.push(quantityEvent);if(pollutionEvent)events.push(pollutionEvent);}previousRegion=region;upstreamFlow=outflow;
    }
