...
export function ensureWaterGovernance(region){
  ...
  const stateDefaults={droughtStage:0,householdRestriction:0,restrictionDurationYears:0,householdHardship:0,necessaryHardship:0,avoidableHardship:0,underinvestment:0,allocationInequity:0,affordabilityBurden:0,grievance:0,migrationPenalty:0,emigrationPush:0,revolutionaryPressure:0,fairness:1};
...
export function waterDemandCaps(region,demand){
  const s=ensureWaterGovernance(region),p=s.policy;
  const discretionaryCut=p.restrictionSeverity*(.08+.42*p.householdPriority);
  return {households:positive(demand.households)*(1-discretionaryCut),agriculture:positive(demand.agriculture),livestock:positive(demand.livestock),industry:positive(demand.industry),controlledEnvironment:positive(demand.controlledEnvironment)};
}
...
function infrastructureUnderinvestment(region){
  ...
  const s=ensureWaterGovernance(region),water=region.waterResources||{};
  ...
  return clamp(gap*stress*(.55+.45*administrativeCapacity(region)));
}
...
export function finaliseWaterGovernance(region,elapsedDays=7){
  ...
  s.householdRestriction=clamp(Math.max(shortage,p.restrictionSeverity*.5));
  ...
  s.affordabilityBurden=clamp(p.scarcityPricing*s.householdRestriction*affordabilitySensitivity);
  ...
  region.report||={};region.report.waterGovernance={workers:0,policy:{...p},droughtStage:s.droughtStage,householdRestriction:s.householdRestriction,restrictionDurationYears:s.restrictionDurationYears,householdHardship:s.householdHardship,necessaryHardship:s.necessaryHardship,avoidableHardship:s.avoidableHardship,underinvestment:s.underinvestment,allocationInequity:s.allocationInequity,affordabilityBurden:s.affordabilityBurden,grievance:s.grievance,migrationPenalty:s.migrationPenalty,emigrationPush:s.emigrationPush,revolutionaryPressure:s.revolutionaryPressure,fairness:s.fairness};
  return s;
}
