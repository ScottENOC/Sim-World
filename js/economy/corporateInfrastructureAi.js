...
function typeDemand(type,region){const assets=ensureCorporateInfrastructure(region).assets;if(assets.some(a=>a.type===type&&a.status!=='destroyed'))return-1;const urban=clamp(Math.log1p(region.population||0)/12),trade=clamp(Math.log1p((region.tradeEconomy?.exportIncomeEma||0)+(region.tradeEconomy?.importSpendEma||0))/8);switch(type){case'port':return region.isCoastal ? .35+trade*.5 : -1;case'mine':return Object.keys(region.resourceDeposits||{}).length ? .25+Object.keys(region.resourceDeposits||{}).length*.04 : -1;case'steelworks':return .18+urban*.35;case'factory':return .2+urban*.45+clamp(region.industrialInvestment?.factoryInvestmentSignal||0)*.55;case'canal':return .1+trade*.35;case'telegraph':return .12+trade*.4+urban*.2;case'power_generation':return .1+urban*.55;case'power_grid':return assets.some(a=>a.type==='power_generation'&&a.status!=='destroyed') ? .5+urban*.3 : .05;case'water_supply':return .15+urban*.55;default:return 0;}}
