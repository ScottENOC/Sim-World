import { commercialRiskPremium } from './monetaryModernisation.js?v=20260918-money1';
...
function updateFirms(region, polity, currentTick, years, rng, events) {
  ...
  for (const firm of s.firms) {
    ...
    const leverage = firm.debtIndex / Math.max(0.01, firm.capitalIndex);const regulation=region.economicRegulation || clamp(s.corporateLaw*.35);const labourPower=clamp(region.medievalSociety?.urban?.guilds || 0);evolveEnterpriseOperatingModel(firm,{publicEnterprise:false,years,profitPressure:clamp(Math.max(0,.06-(firm.profitability||0))+crisis*.35),debtPressure:clamp(leverage),regulation,labourPower,servicePressure:0});const externalityProfile=enterpriseCostExternalityProfile(firm,{publicEnterprise:false});const borrowingRate=Math.max(0,region.monetaryConditions?.commercialBaseRate||0.04)+commercialRiskPremium(region,firm);firm.borrowingRate=borrowingRate;const interestDrag=Math.min(.22,borrowingRate*leverage*.8);const targetProfit = -0.08 + reliability * 0.18 + confidence * 0.12 + s.corporateLaw * 0.08 + sectorFit - crisis * 0.28 + externalityProfile.apparentCostSaving*.22-interestDrag;applyEnterpriseExternalities(region,firm,{scale:Math.min(1,(firm.capitalIndex||0)/8),publicEnterprise:false});
