...
const COAL_PER_AMMONIA=1.75;
const MAX_YIELD_GAIN=.68;
const RESERVE_YEARS=.35;

export function ensureAgriculturalFertiliser(region){
  region.agriculturalFertiliser||={};
  const s=region.agriculturalFertiliser;
  for(const [k,v] of Object.entries({ammoniaExperience:0,fertiliserExperience:0,lastAmmoniaProduced:0,lastFertiliserProduced:0,lastApplied:0,applicationCoverage:0,yieldMultiplier:1,nutrientRunoff:0,waterQualityPenalty:0,electricityLoad:0,feedstockGas:0,feedstockCoal:0,productionCapability:0}))if(!Number.isFinite(s[k]))s[k]=v;
  return s;
}
