...
import { aiSectorOutputMultiplier } from './aiLabour.js?v=20260921-ai-labour2';
...
export function tickIndustrialSupply(region, elapsedDays = 7) {
  ...
  const electric = electricityIndustrialMultiplier(region); const communications = telephoneIndustrialMultiplier(region); const labour = clamp(region.labourRelations?.outputMultiplier ?? 1, .45, 1); const production = industrialProductionMultipliers(region); const ai = aiSectorOutputMultiplier(region, 'manufacturing');
  s.outputCapacity.steel = Math.max(0, base * s.capability.steelmaking * 140 * electric * communications * labour * production.standardisedGoods * ai); s.outputCapacity.machine_components = Math.max(0, base * s.capability.precision_machining * 38 * electric * communications * labour * production.machinery * ai); s.outputCapacity.steam_locomotive = Math.max(0, base * s.capability.locomotive_engineering * 3.2 * electric * communications * labour * production.standardisedGoods * ai); s.outputCapacity.rail_stock = Math.max(0, base * s.capability.rail_vehicle_manufacture * 22 * electric * communications * labour * production.standardisedGoods * ai); s.outputCapacity.motor_vehicle = region.unlockedTechIds?.has(AUTOMOBILE_TECH_ID) ? Math.min(industrialFactoryCapacity(region), Math.max(0, base * s.capability.automotive_engineering * 12 * electric * communications * labour * production.standardisedGoods * ai)) : 0; return s;
}
