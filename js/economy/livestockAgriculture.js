...
export function tickLivestockAgriculture(region,elapsedDays=7){
  ...
  const modernHousing=tech.has?.('industrial_electrification')&&electricity>.12;
  const intensiveCapability=modernHousing?clamp(electricity*.24+machines*.22+vet*.25+literacy(region)*.14+admin(region)*.15):0;
  const intensiveTarget=intensiveCapability*clamp(.15+s.intensificationInvestment*.85);
  const adjust=1-Math.exp(-years*(.12+s.intensificationInvestment*.30));
  ...
  region.stockpile||={};const grain=positive(region.stockpile.staple_grains),feedConsumed=Math.min(grain,feedNeed);if(feedConsumed>0)region.stockpile.staple_grains=Math.max(0,grain-feedConsumed);
  s.feedDemand=feedNeed;s.feedConsumed=feedConsumed;s.feedSatisfaction=feedNeed>0?clamp(feedConsumed/feedNeed):1;

  const crowding=s.intensiveShare,feedStress=1-s.feedSatisfaction,baselineDisease=.06+crowding*.22+feedStress*.12;
  s.veterinaryCapability=vet;s.diseasePressure=clamp(baselineDisease*(1-vet*.62)*(1-s.diseaseResistance*.38),.015,.55);
  const extensiveBase=.82+pasture*.32;
  const geneticGain=1+s.productivityTrait*.24+s.feedEfficiency*.06;
  const intensiveGain=1+s.intensiveShare*.42*s.feedSatisfaction;
  const diseaseLoss=1-s.diseasePressure*.42;
  s.outputMultiplier=clamp(extensiveBase*geneticGain*intensiveGain*diseaseLoss,.55,1.95);

  s.waterDemand=scale*(.006+.018*s.intensiveShare)*Math.max(.05,positive(elapsedDays)/7);
  s.electricityLoad=modernHousing?scale*s.intensiveShare*(.015+.035*electricity)*Math.max(.05,positive(elapsedDays)/7):0;
  s.manureNutrientLoad=clamp(scale*(.018+.050*s.intensiveShare)*(1-vet*.10),0,2.5);
  s.methaneIntensity=clamp(1-s.feedEfficiency*.24+s.intensiveShare*.06,.72,1.12);
  s.antibioticPressure=clamp(s.intensiveShare*s.diseasePressure*(.35+.65*vet),0,.45);
  s.welfarePressure=clamp(s.intensiveShare*(.35+.40*feedStress+.25*(1-vet)),0,.85);
