...
export function tickLivestockAgriculture(region,elapsedDays=7){
  const s=ensureLivestockAgriculture(region),years=Math.max(0,positive(elapsedDays))/DAYS_PER_YEAR,tech=region?.unlockedTechIds||new Set(),electricity=power(region),science=breedingScience(region),vet=veterinaryCapability(region),machines=mechanisation(region),pasture=pastureSignal(region),winter=nuclearWinterEffects(region);
  s.nuclearWinterPastureMultiplier=winter.pastureMultiplier;
  ...
  const crowding=s.intensiveShare,feedStress=1-s.feedSatisfaction,baselineDisease=.06+crowding*.22+feedStress*.12,effectiveVet=vet*(1-s.antibioticResistance*.45);
  ...
  const extensiveBase=.82+pasture*.32,geneticGain=1+s.productivityTrait*.24+s.feedEfficiency*.06,intensiveGain=1+s.intensiveShare*.42*s.feedSatisfaction,diseaseLoss=1-s.diseasePressure*.42;s.outputMultiplier=clamp(extensiveBase*geneticGain*intensiveGain*diseaseLoss,.45,1.95);
  s.waterDemand=scale*(.006+.018*s.intensiveShare)*Math.max(.05,positive(elapsedDays)/7);s.electricityLoad=modernHousing?scale*s.intensiveShare*(.015+.035*electricity)*Math.max(.05,positive(elapsedDays)/7):0;s.manureNutrientLoad=clamp(scale*(.018+.050*s.intensiveShare)*(1-vet*.10),0,2.5);s.methaneIntensity=clamp(1-s.feedEfficiency*.24+s.intensiveShare*.06,.72,1.12);s.welfarePressure=clamp(s.intensiveShare*(.35+.40*feedStress+.25*(1-vet)),0,.85);
