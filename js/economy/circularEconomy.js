...
export function ensureCircularEconomy(region) {
  ...
  s.policy ||= {
    collectionEffort: .18,
    recycledContentStandard: 0,
    landfillDisincentive: .08,
    repairAndReuse: .12,
    recoveryInvestment: .10,
    urbanMiningEffort: 0,
    materialSubstitution: 0,
  };
  for (const key of ['collectionEffort','recycledContentStandard','landfillDisincentive','repairAndReuse','recoveryInvestment','urbanMiningEffort','materialSubstitution']) {
    if (!Number.isFinite(s.policy[key])) s.policy[key] = 0;
  }
  ...
  s.inUse ||= {}; s.scrap ||= {}; s.landfill ||= {}; s.pendingUse ||= {}; s.recovered ||= {}; s.losses ||= {}; s.materials ||= {};
...
export function setCircularEconomyPolicy(region, patch = {}) {
  ...
  for (const key of ['collectionEffort','recycledContentStandard','landfillDisincentive','repairAndReuse','recoveryInvestment','urbanMiningEffort','materialSubstitution']) {
    if (patch[key] !== undefined) p[key] = clamp(patch[key]);
  }
  return { ...p };
}
...
export function recordMaterialUse(region, material, amount, sector = 'general') {
  ...
  s.materials[material].lastSector = sector;
  return used;
}

export function recordMaterialDiscard(region, material, amount, { recoverableFraction = 1 } = {}) {
  if (!MATERIAL_SPECS[material]) return 0;
  const discarded = nonNegative(amount);
  if (!discarded) return 0;
  const s = ensureCircularEconomy(region);
  const recoverable = discarded * clamp(recoverableFraction);
...
function depositReserve(deposit) {
  ...
  if (Array.isArray(deposit.tiers)) {
    ...
    for (const tier of deposit.tiers) {
      remaining += nonNegative(tier.remainingStock);
      initial += nonNegative(tier.initialStock);
    }
...
export function materialReserveStatus(region, material) {
  ...
  if (!spec) return { fraction: 0, remaining: 0, initial: 0 };
  let remaining = 0, initial = 0;
  const deposits = region?.deposits || region?.resourceDeposits || {};
  for (const key of spec.depositKeys) {
    const d = depositReserve(deposits[key]);
...
function bootstrapKnowledge(region) {
  ...
  const ind = industry(region), lit = literacy(region), elec = electricity(region);
...
function updateCapability(region, s, years) {
  ...
  const recycling = tech(region,CIRCULAR_ECONOMY_TECH_IDS.RECYCLING) ? 1 : 0;
  const advanced = tech(region,CIRCULAR_ECONOMY_TECH_IDS.ADVANCED_RECOVERY) ? 1 : 0;
  const ecodesign = tech(region,CIRCULAR_ECONOMY_TECH_IDS.ECODESIGN) ? 1 : 0;
  const urbanMining = tech(region,CIRCULAR_ECONOMY_TECH_IDS.URBAN_MINING) ? 1 : 0;
  const closedLoop = tech(region,CIRCULAR_ECONOMY_TECH_IDS.CLOSED_LOOP) ? 1 : 0;
  const advancedSub = tech(region,CIRCULAR_ECONOMY_TECH_IDS.ADVANCED_SUBSTITUTION) ? 1 : 0;
  const targets = {
...
export function plasticDemandMultiplier(region) {
  const s = ensureCircularEconomy(region);
  return 1 - clamp(s.plasticSubstitution) * .95;
}

function tickMaterial(region, s, material, years) {
  const spec = MATERIAL_SPECS[material];
  const added = nonNegative(s.pendingUse[material]);
  s.pendingUse[material] = 0;
  ...
  const recoveredFromScrap = scrapProcessed * recoveryEfficiency;
  const processLoss = scrapProcessed - recoveredFromScrap;
  s.scrap[material] = Math.max(0, s.scrap[material] - scrapProcessed);

  // Early economies lose metals into mixed waste. Once urban mining exists,
  // that old landfill becomes a low-grade ore body rather than a permanent deletion timer.
  ...
  const recoveredFromLandfill = landfillProcessed * landfillEfficiency;
  const landfillResidue = landfillProcessed - recoveredFromLandfill;
  ...
  s.recovered[material] += recovered;
  region.stockpile ||= {};
  region.stockpile[spec.stockKey] = nonNegative(region.stockpile[spec.stockKey]) + recovered;
  ...
  s.materials[material] = {
    ...(s.materials[material]||{}), added, retired, recovered,
    recoveredFromScrap, recoveredFromLandfill, landfillProcessed,
    reserveFraction: reserve.fraction, secondaryShare, depletionPressure,
    substitutionPotential: substitution, security, electricityCost,
  };
...
export function tickCircularEconomy(region, elapsedDays = 7) {
  ...
  const results = {};
  for (const material of Object.keys(MATERIAL_SPECS)) results[material] = tickMaterial(region,s,material,years);
  ...
  region.report ||= {};
  region.report.circularEconomy = circularEconomySummary(region);
  return s;
}
...
export function circularEconomySummary(region) {
  ...
  return {
    ...
    circularityRate:s.circularityRate,
    virginDependence:s.virginDependence,
    materialSecurity:s.materialSecurity,
    durableControlMargin:s.durableControlMargin,
    electricityLoad:s.electricityLoad,
    plasticSubstitution:s.plasticSubstitution,
    materials:Object.fromEntries(Object.entries(s.materials).map(([k,v])=>[k,{...v,inUse:s.inUse[k],scrap:s.scrap[k],landfill:s.landfill[k],cumulativeRecovered:s.recovered[k],cumulativePermanentLosses:s.losses[k]}])),
  };
}
