...
function importExposure(region){const im=Math.max(0,region.tradeEconomy?.weeklyImports||0),ex=Math.max(0,region.tradeEconomy?.weeklyExports||0);return pct(im/Math.max(1,im+ex));}
...
function assess(region,s){
  ...
  const wageRelief=clamp(Math.min(floor,affordable)*.48+standards*.18),protectionJobs=clamp(s.policy.importProtection*importExposure(region)*industry*.12,0,.08),protectionCost=clamp(s.policy.importProtection*(.08+importExposure(region)*.24),0,.22);
  const workplace=pct((region.enterpriseExternalities?.labourHarm||0)*.55+(1-safety)*industry*.18),wageCostPressure=pct(hardship*.38+food*.3+protectionCost*.24-wageRelief),insecurity=pct(unemployment*.5+failures*.28+trade*.2-protectionJobs*.65-relief*.12);
...
export function tickLabourRelations(region,currentTick=0,elapsedDays=7,{isPlayer=false}={}){
  ...
  const pressureTarget=pct(a.strikeBase+s.grievance*.28);s.strikePressure+=(pressureTarget-s.strikePressure)*clamp(weeks/8);s.hiringPenalty=a.hiringPenalty;s.protectionJobs=a.protectionJobs;s.protectionCost=a.protectionCost;
  const events=[];
