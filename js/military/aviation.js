...
import { airCombatAssessment, recordHostileAirMission } from './airspaceGreyZone.js?v=20260923-aircombat1';
...
export function tickAviation(regions,currentTick,elapsedDays=7,rng=Math.random,options={}){
  ...
  for(const region of regions){
    ...
    for(const a of av.aircraft){
      ...
      const detectedOrEngaged=risk>0;
      if(rng()<risk){const reliability=clamp(a.designStats?.reliability||0),damage=(.18+rng()*.62)*(1-reliability*.22);const status=damageAircraft(a,damage);if(status==='destroyed')resolveAircraftCrewLoss(region,a,{rng});events.push({type:status==='destroyed'?'aircraft_shot_down':'aircraft_damaged',aircraftId:a.id,targetRegionId:target.id,damage,airspaceCombat:combatContext});}
      if(a.status!=='destroyed'&&a.mission===AIR_MISSIONS.SCOUT){const nav=clamp(a.designStats?.radioNavigation||0),range=clamp(a.designStats?.range||0),crew=aircraftCrewReadiness(a);region.airRecon ||= {};region.airRecon[target.id]={observedTick:currentTick,confidence:clamp((.35+a.pilotExperience*.30+a.condition*.20+nav*.10+range*.05)*(.72+.28*crew)*(combatContext?.reconConfidenceMultiplier??1))};events.push({type:'aerial_reconnaissance',aircraftId:a.id,targetRegionId:target.id,airspaceCombat:combatContext});}
      if(a.status!=='destroyed'&&a.mission===AIR_MISSIONS.ATTACK&&has(region,AERIAL_BOMBING_TECH_ID)){const payload=clamp(a.designStats?.payload??.18),firepower=clamp(a.designStats?.firepower??.15),crew=aircraftCrewReadiness(a),strikeMultiplier=combatContext?.strikeEffectivenessMultiplier??1;target.warDamage ||= {infrastructureDamage:0,bombardmentWeeks:0};target.warDamage.infrastructureDamage+=.006*a.condition*(.45+payload*1.6+firepower*.35)*(.68+.32*crew)*strikeMultiplier;events.push({type:'aerial_attack',aircraftId:a.id,targetRegionId:target.id,payload,strikeMultiplier,airspaceCombat:combatContext});if(foreignTarget)recordHostileAirMission(target,actorId(region),{detected:detectedOrEngaged,currentTick});}
