...
export function tickModernEnergyBreakthroughs(regions,currentTick,rng=Math.random,elapsedDays=7){
  ...
  for(const r of world){const t=r.unlockedTechIds;if(t?.has?.(SHALLOW_OIL_DRILLING_TECH_ID)||t?.has?.(ELECTRICAL_GENERATION_TECH_ID)){energyActive=true;break;}}
  ...
  for(const region of world){
    ...
    for(const [key,id,type,title,message] of attempts){
      ...
      // Keep RNG consumption for impossible checks so this optimisation does not
      // reorder seeded histories.
    ...
    tickDirectedEnergyDefence(region,elapsedDays);
  }

  // Space-race entry is separately gated by strategic missile capability. Only
  // run the families that require an existing programme after one actually exists.
