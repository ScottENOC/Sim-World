...
export function medicalBreakthroughChance(region,regionsById,definition,knownTechIds=region.unlockedTechIds){
  if(knownTechIds?.has(definition.id))return 0;
  if(!definition.requires.every(id=>knownTechIds?.has(id)))return 0;
  const readiness=clamp(definition.readiness(region));
  const researchMultiplier=1+aiResearch(region)*0.85;
  const independent=chanceFrom(readiness,definition.base)*researchMultiplier;
  return clamp(independent+diffusion(region,regionsById,definition.id)*0.03);
}
