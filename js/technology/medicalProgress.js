...
export function resolveMilitaryCasualties(region,grossLosses,{deployedPersonnel=0,homeCare=false,logistics=1}={}){const gross=Math.max(0,Math.round(grossLosses||0));if(!gross)return {gross:0,deaths:0,wounded:0,survivingWounded:0,returnedEventually:0,medicalEffect:0,capacityRatio:0};const immediateDeaths=Math.round(gross*0.34);const wounded=Math.max(0,gross-immediateDeaths);const effect=militaryMedicalEffect(region);const fieldCapacity=Math.max(1,deployedPersonnel*(0.008+effect*0.024)*clamp(logistics,0.15,1.2));const hospitalCapacity=homeCare?Math.max(0,Number(region.publicHealth?.operationalBeds)||0)*0.8:Math.max(0,Number(region.publicHealth?.operationalBeds)||0)*0.12;const capacityRatio=clamp((fieldCapacity+hospitalCapacity)/Math.max(1,wounded));const untreatedSurvival=0.42;const treatedSurvival=clamp(untreatedSurvival+effect*0.72,0,0.95);const survivalRate=untreatedSurvival+(treatedSurvival-untreatedSurvival)*capacityRatio;const survivingWounded=Math.round(wounded*survivalRate);const deaths=Math.min(gross,immediateDeaths+(wounded-survivingWounded));return {gross,deaths,wounded,survivingWounded,returnedEventually:survivingWounded,medicalEffect:effect,capacityRatio};}
