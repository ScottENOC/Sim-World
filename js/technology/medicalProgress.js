...
export function militaryMedicalEffect(region){const c=medicalCapabilities(region),h=region.publicHealth||{};const hospitalBase=clamp((Number(h.staffingRatio)||0)*(Number(h.fundingRatio)||0));let treatment=0.03+(c.professionalMedicine?0.06:0)+(c.anatomy?0.04:0)+(c.nursing?0.10:0)+(c.antisepsis?0.16:0)+(c.germTheory?0.05:0)+(c.antibiotics?0.23:0);const evacuation=clamp(0.25+stateCapacity(region)*0.35+(region.telephoneNetwork?.coverage||0)*0.15+(region.railway?.networkLevel||0)*0.15);const ai=aiHealthcare(region);return clamp(treatment*(0.45+hospitalBase*0.35+evacuation*0.20)*(1+ai*.45),0,0.78);}
