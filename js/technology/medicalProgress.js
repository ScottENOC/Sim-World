...
import { aiSectorOutputMultiplier } from '../economy/aiLabour.js?v=20260921-ai-labour1';
import { antibioticTreatmentEffect } from '../society/antimicrobialResistance.js?v=20260922-amr1';
...
function aiResearch(region){return clamp(aiSectorOutputMultiplier(region,'research')-1,0,.75);}
function aiHealthcare(region){return clamp(aiSectorOutputMultiplier(region,'healthcare')-1,0,.75);}
...
export function medicalBreakthroughChance(region,regionsById,definition,knownTechIds=region.unlockedTechIds){if(knownTechIds?.has(definition.id))return 0;if(!definition.requires.every(id=>knownTechIds?.has(id)))return 0;const readiness=clamp(definition.readiness(region));const researchMultiplier=1+aiResearch(region)*0.85;const independent=chanceFrom(readiness,definition.base)*researchMultiplier;return clamp(independent+diffusion(region,regionsById,definition.id)*0.03);}
export function tickMedicalBreakthroughs(regions,currentTick,rng=Math.random,elapsedDays=7){const weekScale=Math.max(0.01,elapsedDays/7),regionsById=new Map(regions.map(r=>[r.id,r]));const knownAtStart=new Map(regions.map(r=>[r.id,new Set(r.unlockedTechIds||[])])),discoveries=[];for(const d of DEFINITIONS)for(const region of regions){const p=medicalBreakthroughChance(region,regionsById,d,knownAtStart.get(region.id));const adjusted=1-Math.pow(1-clamp(p),weekScale);if(rng()<adjusted)discoveries.push({region,d});}const events=[];for(const {region,d} of discoveries){region.unlockedTechIds.add(d.id);events.push({type:'medical_breakthrough',techId:d.id,title:d.label,regionId:region.id,regionName:region.name,tick:currentTick,message:`${region.name} has developed ${d.label.toLowerCase()}.`});}events.push(...tickPreservationAndEnvironmentalHealthBreakthroughs(regions,currentTick,rng,elapsedDays));tickWorldOzoneLayer(regions,elapsedDays);tickOzoneDiplomacy(regions,currentTick,elapsedDays);return events;}
export function vaccinationProtection(region,pathogenId){const c=medicalCapabilities(region);if(!c.vaccination||pathogenId!=='smallpox')return 0;const admin=clamp(region.publicHealth?.publicHealthAdministration||0);const coverage=clamp(0.08+admin*0.62+stateCapacity(region)*0.18);const efficacy=c.germTheory?0.82:0.68;return clamp(coverage*efficacy*(1+aiHealthcare(region)*0.25),0,0.88);}
export function treatmentKnowledgeEffect(region,pathogenId){const c=medicalCapabilities(region);let e=0.08+(c.professionalMedicine?0.10:0)+(c.anatomy?0.05:0)+(c.nursing?0.11:0)+(c.antisepsis?0.14:0)+(c.germTheory?0.08:0);if(c.antibiotics&&(pathogenId==='enteric'||pathogenId==='plague'||pathogenId==='respiratory'))e+=0.28*antibioticTreatmentEffect(region,pathogenId);return clamp(e*(1+aiHealthcare(region)*0.55),0,0.82);}
export function preventionKnowledgeEffect(region){const c=medicalCapabilities(region);const base=(c.antisepsis?0.08:0)+(c.germTheory?0.18:0)+(c.professionalMedicine?0.03:0);return clamp(base*(1+aiHealthcare(region)*0.35),0,0.42);}
export function militaryMedicalEffect(region){const c=medicalCapabilities(region),h=region.publicHealth||{};const hospitalBase=clamp((Number(h.staffingRatio)||0)*(Number(h.fundingRatio)||0));const antibioticEffect=c.antibiotics?(antibioticTreatmentEffect(region,'enteric')+antibioticTreatmentEffect(region,'respiratory'))*.5:0;let treatment=0.03+(c.professionalMedicine?0.06:0)+(c.anatomy?0.04:0)+(c.nursing?0.10:0)+(c.antisepsis?0.16:0)+(c.germTheory?0.05:0)+(c.antibiotics?0.23*antibioticEffect:0);const evacuation=clamp(0.25+stateCapacity(region)*0.35+(region.telephoneNetwork?.coverage||0)*0.15+(region.railway?.networkLevel||0)*0.15);const ai=aiHealthcare(region);return clamp(treatment*(0.45+hospitalBase*0.35+evacuation*0.20)*(1+ai*.45),0,0.78);}
export function resolveMilitaryCasualties(region,grossLosses,{deployedPersonnel=0,homeCare=false,logistics=1}={}){const gross=Math.max(0,Math.round(grossLosses||0));if(!gross)return {gross:0,deaths:0,wounded:0,survivingWounded:0,returnedEventually:0,medicalEffect:0,capacityRatio:0};const immediateDeaths=Math.round(gross*0.34),wounded=Math.max(0,gross-immediateDeaths),effect=militaryMedicalEffect(region);const aiCapacity=1+aiHealthcare(region)*0.65;const fieldCapacity=Math.max(1,deployedPersonnel*(0.008+effect*0.024)*clamp(logistics,0.15,1.2)*aiCapacity);const hospitalCapacity=(homeCare?Math.max(0,Number(region.publicHealth?.operationalBeds)||0)*0.8:Math.max(0,Number(region.publicHealth?.operationalBeds)||0)*0.12)*aiCapacity;const capacityRatio=clamp((fieldCapacity+hospitalCapacity)/Math.max(1,wounded));const untreatedSurvival=0.42,treatedSurvival=clamp(untreatedSurvival+effect*0.72,0,0.95),survivalRate=untreatedSurvival+(treatedSurvival-untreatedSurvival)*capacityRatio;const survivingWounded=Math.round(wounded*survivalRate),deaths=Math.min(gross,immediateDeaths+(wounded-survivingWounded));return {gross,deaths,wounded,survivingWounded,returnedEventually:survivingWounded,medicalEffect:effect,capacityRatio};}
