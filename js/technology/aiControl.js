...
function inferredAccess(region,s){
  ...
  // Strategic access is not inferred from ordinary adoption. It must come from an explicit
  // integration decision or another strategic subsystem.
...
export function tickAiControl(region,elapsedDays=7){
  ...
  // This is an internal hazard, not a player-facing score. Capability is beneficial elsewhere;
  // risk appears when rapid capability growth combines with autonomy, consequential access and lagging controls.
...
export function aiControlSummary(region){
  ...
  return {
    ...
    // Advisors can reason from these underlying observations. UI should avoid rendering
    // lossOfControlHazard as a boss meter.
