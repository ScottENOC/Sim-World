...
import { tickAviation, syncNextAircraftId } from './military/aviation.js?v=20260918-aviation1';
...
import { tickCorporateCapitalWithDistress, resolvePlayerFirmDistressEvent } from './economy/corporateDistressRuntime.js?v=20260917-bankruptcy1';
...
async function main() {
  ...
  clock.onTick(async (time) => {
    ...
    const aviationEvents = profiler.measure('Aviation', () => tickAviation(regions, calendarWeek, time.elapsedDays, Math.random));
    ...
    const capitalEvents = profiler.measure('Corporate capital', () => tickCorporateCapitalWithDistress(regions, polities, calendarWeek, time.elapsedDays, Math.random, { playerPolityId: activePlayerPolityId }));
...
function showNextEvent(clock, eventQueue) {
  ...
  if (event.type === 'commercial_firm_distress') {
    const options = document.getElementById('event-options');
