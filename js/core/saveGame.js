import { ensureCivilianTransport } from '../economy/civilianTransport.js';
...
function regionSnapshot(region) {
  const state = {};
  for (const [key, value] of Object.entries(region)) if (!STATIC_REGION_KEYS.has(key) && key !== 'knowledge') state[key] = encode(value);
  state.knowledge = encode({ ownerId: region.knowledge.ownerId, observations: region.knowledge.observations,
    knownSubjectIds: region.knowledge.knownSubjectIds, directContactIds: region.knowledge.directContactIds,
...
function restoreRegion(region, saved) {
  ...
  for (const key of Object.keys(region)) if (!STATIC_REGION_KEYS.has(key) && key !== 'knowledge') delete region[key];
  const state = decode(saved.state); const knowledgeState = state.knowledge; delete state.knowledge; Object.assign(region, state);
  for (const key of Object.keys(knowledge)) delete knowledge[key]; Object.assign(knowledge, knowledgeState); region.knowledge = knowledge;
...
export function restoreGameSnapshot(snapshot, { regions, seaRegions, polities, religiousWorld, agreements, activeRaids, activeCampaigns, activeWars = null, fleets = null, internationalOrganisations = null, clock, fogOfWar }) {
  ...
  if (snapshot.worldRegionIds.length !== expectedIds.length || snapshot.worldRegionIds.some((id, index) => id !== expectedIds[index])) throw new Error('This save belongs to a different version of the world map.');
  const savedById = new Map(snapshot.regions.map((entry) => [entry.id, entry]));
  for (const region of regions) { const saved = savedById.get(region.id); if (!saved) throw new Error(`Save is missing region ${region.id}.`); restoreRegion(region, saved); }
  const savedSeas = new Map((snapshot.seaRegions || []).map((entry) => [entry.id, entry]));
  for (const sea of seaRegions) { const saved = savedSeas.get(sea.id); if (!saved) throw new Error(`Save is missing sea region ${sea.id}.`); sea.fish = decode(saved.fish); }
  ...
  clock.setResolution(snapshot.clock.resolution || 'month'); clock._pendingResponseRequired = 0;
  ...
  return { playerRegionId: snapshot.playerRegionId, playerPolityId: snapshot.playerPolityId || null, savedAt: snapshot.savedAt, fleetsRestored, internationalOrganisationsRestored };
}
...
export function saveSummary(storage = localStorage) { try { const save = readSave(storage); return save ? { savedAt: save.savedAt, tickIndex: save.clock?.tickIndex || 0 } : null; } catch { return { invalid: true }; } }
