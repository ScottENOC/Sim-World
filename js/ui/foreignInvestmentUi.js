...
import { PROCUREMENT_POLICIES, describeProcurementConsequence, ensureProcurementPolicy, setInfrastructureProcurementPolicy } from '../economy/industrialSupply.js';
...
const PROCUREMENT_LABELS = Object.freeze({
  [PROCUREMENT_POLICIES.BEST_AVAILABLE]: 'Best available',
  [PROCUREMENT_POLICIES.PREFER_DOMESTIC]: 'Prefer domestic',
  [PROCUREMENT_POLICIES.DOMESTIC_ONLY]: 'Domestic only',
});
...
function regionById(sim, id) { return sim?.regions?.find((region) => region.id === id) || null; }
...
function policyOptions(selected) {
  return Object.values(FOREIGN_INVESTMENT_POLICIES).map((value) => `<option value="${value}" ${selected === value ? 'selected' : ''}>${POLICY_LABELS[value]}</option>`).join('');
}
function procurementOptions(selected) {
  return Object.values(PROCUREMENT_POLICIES).map((value) => `<option value="${value}" ${selected === value ? 'selected' : ''}>${PROCUREMENT_LABELS[value]}</option>`).join('');
}
function offerCard(sim, offer) {
  const host = regionById(sim, offer.hostRegionId);
  const materials = Object.entries(offer.materials || {}).map(([resource, amount]) => `${fmt(amount)} ${resource.replaceAll('_', ' ')}`).join(' · ');
...
function nationalisationControls(sim, region, asset, treasuryRegion) {
  ...
  const value = Math.max(0, Number(asset.value) || 0), treasury = Math.max(0, Number(treasuryRegion?.treasury) || 0);
  const fullDisabled = treasury + 1e-9 < value ? 'disabled' : '', halfDisabled = treasury + 1e-9 < value * .5 ? 'disabled' : '';
...
function assetCard(sim, region, asset, treasuryRegion) {
  const owner = polityName(sim, asset.ownerPolityId), operator = polityName(sim, asset.operatorPolityId), financier = polityName(sim, asset.financedByPolityId || asset.ownerPolityId), builder = polityName(sim, asset.builtByPolityId || asset.technologySourcePolityId || asset.ownerPolityId);
  const condition = pct(asset.condition ?? 1), capacity = pct(asset.effectiveCapacity ?? (asset.status === 'construction' ? asset.progress || 0 : 1));
...
function renderPanel(sim, panel) {
  ...
  if (!polity) { panel.innerHTML = '<h3>Foreign investment</h3><p class="advisor-note">No player government is active.</p>'; return; }
  const policy = ensureInvestmentPolicy(polity), procurement = ensureProcurementPolicy(polity).infrastructure, treasuryRegion = playerTreasuryRegion(sim, territories);
  const proposals = territories.flatMap((region) => ensureCorporateInfrastructure(region).proposals).filter((proposal) => proposal.status === 'pending').sort((a, b) => (a.createdTick || 0) - (b.createdTick || 0));
  const assets = territories.flatMap((region) => ensureCorporateInfrastructure(region).assets.map((asset) => ({ region, asset }))).filter(({ asset }) => asset.status !== 'destroyed').sort((a, b) => Number(b.asset.foreignOwner) - Number(a.asset.foreignOwner) || a.region.name.localeCompare(b.region.name));
  panel.innerHTML = `
    ...
    <h3>Government market policy</h3>
    <p class="advisor-note">Investment policy governs whether foreign firms may own or operate assets here. Procurement policy is separate: it governs which suppliers the state prefers when buying infrastructure. Strategic security restrictions still apply to foreign tenders.</p>
    ...
    <label class="advisor-field"><span>Strategic infrastructure investment</span><select data-investment-policy="strategic">${policyOptions(policy.strategic)}</select></label>
    <p class="advisor-note">${POLICY_HELP[policy.strategic]}</p>
    <label class="advisor-field"><span>Government infrastructure procurement</span><select data-procurement-policy>${procurementOptions(procurement)}</select></label>
    <p class="advisor-note">${describeProcurementConsequence(polity)}</p>
    ...
    <p class="advisor-note">Closing the market does not create an abstract penalty. It removes foreign capital and expertise from the projects available to the realm. Domestic procurement can be slower when local industry is weak, but repeated domestic work builds experience instead of granting a flat bonus.</p>
  ...
  `;
  panel.querySelectorAll('[data-investment-policy]').forEach((select) => select.addEventListener('change', () => { setForeignInvestmentPolicy(polity, { [select.dataset.investmentPolicy]: select.value }); renderPanel(sim, panel); }));
  panel.querySelector('[data-procurement-policy]')?.addEventListener('change', (event) => { setInfrastructureProcurementPolicy(polity, event.currentTarget.value); renderPanel(sim, panel); });
  panel.querySelectorAll('[data-investment-accept]').forEach((button) => button.addEventListener('click', () => {
    const hostRegion = regionById(sim, button.dataset.hostRegion); if (!hostRegion) return;
  ...
  panel.querySelectorAll('[data-investment-reject]').forEach((button) => button.addEventListener('click', () => { const hostRegion = regionById(sim, button.dataset.hostRegion); if (!hostRegion) return; resolveCorporateInfrastructureOffer({ offerId: button.dataset.investmentReject, hostRegion, hostPolity: polity, regions: sim.regions, polities: sim.polities, choice: 'reject', currentTick: sim.clock?.tickIndex || 0 }); lastActionMessage = 'The concession proposal has been rejected.'; renderPanel(sim, panel); }));
  panel.querySelectorAll('[data-nationalise-asset]').forEach((button) => button.addEventListener('click', () => {
    const hostRegion = regionById(sim, button.dataset.hostRegion), asset = ensureCorporateInfrastructure(hostRegion || {}).assets.find((entry) => entry.id === button.dataset.nationaliseAsset); if (!hostRegion || !asset || !treasuryRegion) return;
    const ownerPolity = sim.polities.find((entry) => entry.id === asset.ownerPolityId) || null, compensationShare = Number(button.dataset.compensation);
...
function maybeMount(sim) {
  const content = document.getElementById('advisor-content'), treasurer = document.querySelector('[data-advisor="treasurer"]');
  if (!content || !treasurer?.classList.contains('active')) return;
  let panel = document.getElementById('foreign-investment-advisor-panel');
  if (!panel) { panel = document.createElement('section'); panel.id = 'foreign-investment-advisor-panel'; panel.className = 'advisor-section foreign-investment-advisor-panel'; content.appendChild(panel); }
  renderPanel(sim, panel);
}
export function installForeignInvestmentUi(sim = window.__worldsim) {
  const content = document.getElementById('advisor-content'); if (!sim || !content || content.dataset.foreignInvestmentUiInstalled === 'true') return false;
  content.dataset.foreignInvestmentUiInstalled = 'true'; let queued = false;
  const schedule = () => { if (queued) return; queued = true; queueMicrotask(() => { queued = false; maybeMount(sim); }); };
  new MutationObserver(schedule).observe(content, { childList: true, subtree: false });
  document.getElementById('advisor-tabs')?.addEventListener('click', schedule); document.getElementById('btn-council')?.addEventListener('click', schedule); schedule(); return true;
}
