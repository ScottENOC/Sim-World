...
export class LocalRegionView{
  ...
  draw(){if(!this.scene||!this.ctx)return;this._resize();const ctx=this.ctx;ctx.clearRect(0,0,this.w,this.h);ctx.fillStyle='#10141c';ctx.fillRect(0,0,this.w,this.h);
    ...
    ctx.globalAlpha=1;
    const showSite=(s)=>this.mode==='overview'||this.mode==='control'||this.mode==='military'&&(['fort','port','city','principal_settlement','town'].includes(s.type)||s.garrisonPersonnel>0)||this.mode==='economy'&&(['mine','port','principal_settlement','town','city','great_temple','monumental_tomb','ceremonial_complex','monumental_statue'].includes(s.type));
    for(const s of this.scene.sites.filter(showSite)){const [x,y]=this._project([s.lon,s.lat]);let r=['city','principal_settlement'].includes(s.type)?7:s.type==='port'?6:s.type==='mine'?5:4;
